...
TBG_gpusPerNode=`if [ $TBG_tasks -gt 2 ] ; then echo 8; else echo 2; fi`
...
#if [ $? -eq 0 ] ; then
  mpiexec --prefix $MPIHOME -x LIBRARY_PATH -x LD_LIBRARY_PATH -tag-output --display-map -am !TBG_dstPath/tbg/openib.conf --mca mpi_leave_pinned 0 -npernode !TBG_gpusPerNode -n !TBG_tasks !TBG_dstPath/tbg/gpu_numa_aware !TBG_dstPath/picongpu/bin/picongpu !TBG_author !TBG_programParams | tee output
#fi
