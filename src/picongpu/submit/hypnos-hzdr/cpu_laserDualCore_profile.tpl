...
TBG_gpusPerNode=`if [ $TBG_tasks -gt 32 ] ; then echo 32; else echo $TBG_tasks; fi`

#number of cores per parallel node / default is 2 cores 
...
#if [ $? -eq 0 ] ; then
  mpiexec --prefix $MPIHOME -x LIBRARY_PATH -x LD_LIBRARY_PATH -tag-output --display-map -am !TBG_dstPath/tbg/openib.conf --mca mpi_leave_pinned 0 -npernode !TBG_gpusPerNode -n !TBG_tasks !TBG_dstPath/tbg/numa_aware_dualCore !TBG_dstPath/picongpu/bin/picongpu !TBG_author !TBG_programParams | tee output
#fi
